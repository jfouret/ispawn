...
def local_config(cert_dir):
    ...
    return ProxyConfig(
        mode=ProxyMode.LOCAL.value, domain="test.local", cert_dir=str(cert_dir)
    )
...
def test_setup_remote_certificates_directory(cert_service, remote_config):
    ...
    assert oct(cert_dir.stat().st_mode)[-3:] == "700", (
...
def test_setup_certificates_no_cert_dir(cert_service):
    ...
    with pytest.raises(
        CertificateError, match="Certificate directory not specified"
    ):
...
def test_setup_certificates_letsencrypt_no_email(cert_service, cert_dir):
    ...
    with pytest.raises(
        CertificateError, match="Email required for Let's Encrypt certificates"
    ):
        cert_service.setup_certificates(config)
...
def test_setup_remote_certificates_letsencrypt():
    ...
    """
    pytest.skip(
        "Integration test requiring public domain and Let's Encrypt access"
    )
