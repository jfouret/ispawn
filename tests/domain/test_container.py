...
def test_container_basic_properties(container_config, valid_args):
    ...
    assert (
        ...
        == f"{valid_args['name_prefix']}-{valid_args['name']}"
    )
...
def test_container_labels(container_config, valid_args):
    ...
    for service in valid_args["services"]:
        if service == Service.RSTUDIO:
            assert (
                f"traefik.http.services.rstudio-{valid_args['name']}.loadbalancer.server.port=8787"
                in labels
            )
        elif service == Service.JUPYTER:
            assert (
                f"traefik.http.services.jupyter-{valid_args['name']}.loadbalancer.server.port=8888"
                in labels
            )
        elif service == Service.VSCODE:
            assert (
                f"traefik.http.services.vscode-{valid_args['name']}.loadbalancer.server.port=8842"
                in labels
            )
    ...
    cert_label = (
        ...
        if valid_args["cert_mode"] == "letsencrypt"
        else ".tls=true"
    )
...
def test_container_volume_mounts(container_config, valid_args):
    ...
    if valid_args["volumes"] == 0:
        ...
        expected_mount = (
            f"{volume}:{valid_args['host_prefix']}/{volume.lstrip('/')}"
        )
...
def test_container_service_urls(container_config, valid_args):
    ...
    for service in valid_args["services"]:
        ...
        if service == Service.JUPYTER:
            assert (
                urls[service]
                == f"https://jupyter-{valid_args['name']}.{valid_args['domain']}?token={valid_args['password']}"
            )
        elif service == Service.RSTUDIO:
            assert (
                urls[service]
                == f"https://rstudio-{valid_args['name']}.{valid_args['domain']}"
            )
        elif service == Service.VSCODE:
            assert (
                urls[service]
                == f"https://vscode-{valid_args['name']}.{valid_args['domain']}"
            )
...
def test_container_config_invalid_fields(
    valid_args, invalid_field, invalid_value, expected_error
):
