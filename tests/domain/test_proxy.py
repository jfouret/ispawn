...
def test_valid_proxy_config_creation(valid_config_params):
    ...
    if params["mode"] == "remote":
        ...
        assert config.requires_email == (
            params.get("cert_mode") == "letsencrypt"
        )
    else:
...
def test_invalid_proxy_config_creation(invalid_config_params):
    ...
    with pytest.raises(
        invalid_config_params["error_type"],
        match=invalid_config_params["error_msg"],
    ):
...
@pytest.mark.parametrize(
    "config1_id,config2_id,should_be_equal",
    [
...
def test_proxy_config_equality(
    config_map, config1_id, config2_id, should_be_equal
):
