...
def update_versions_json(site_dir, version, is_latest=False, base_url=None):
    ...
    latest_versions = [v for v in versions_data if v.get("version") == "latest"]
...
def main():
    ...
    parser.add_argument(
        "--site-dir", required=True, help="Path to the site directory"
    )
    parser.add_argument(
        "--is-latest",
        action="store_true",
    ...
    update_versions_json(
        args.site_dir, args.version, args.is_latest, args.base_url
    )
